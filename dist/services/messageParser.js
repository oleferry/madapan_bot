...
// ── Helpers ───────────────────────────────────────────────────────────────────
function resolveProductAlias(text) {
    const lower = text.toLowerCase().trim();
    // Sort aliases longest first to avoid prefix clashes
    const entries = Object.entries(PRODUCT_ALIASES).sort(([, a], [, b]) => Math.max(...b.map((s) => s.length)) - Math.max(...a.map((s) => s.length)));
...
function extractDate(message, now) {
    const lower = message.toLowerCase();
    // "para el lunes", "para el martes", etc.
    const paraElMatch = lower.match(/para\s+(?:el\s+)?(lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)/i);
    if (paraElMatch) {
        try {
            return (0, dates_1.getDateForDayName)(paraElMatch[1], now);
        }
        catch {
            return null;
        }
    ...
    if (/ma[ñn]ana/.test(lower)) {
        return (0, dates_1.getTomorrowDate)();
    }
    if (lower.includes('hoy')) {
        return (0, dates_1.getTodayDate)();
    }
...
// ── Main export ───────────────────────────────────────────────────────────────
function parseCustomerMessage(message, now) {
